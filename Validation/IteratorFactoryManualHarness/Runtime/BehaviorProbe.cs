...
using System.IO;
using IteratorFactoryManualFixture;
using UnityEngine;
...
namespace RecoveryValidation
{
    public static class BehaviorProbe
    {
        ...
        {
            var owner = new ManualOwner { Marker = 17 };
            var first = (ManualEnumerator)owner.Create();
            var second = (ManualEnumerator)owner.Create();
            var observations = new List<object>
            {
                ...
                Row("first", "owner-captured", ReferenceEquals(first.Owner, owner)),
                Row("second", "owner-captured", ReferenceEquals(second.Owner, owner)),
                Row("first", "current-owner", ReferenceEquals(first.Current, owner)),
                Row("first", "completed", !first.MoveNext()),
            ...
            var other = new ManualOwner { Marker = -17 };
            var third = (ManualEnumerator)other.Create();
            ...
            File.WriteAllText(path, ReportJson.Encode(new Dictionary<string, object>
            {
                ...
                { "stage", stage }, { "profile", "iterator-factory-manual" },
