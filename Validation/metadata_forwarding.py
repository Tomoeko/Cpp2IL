"""Finite behavior oracle for metadata-guarded static forwarding."""
...
def observations():
    ...
    return [
        {"kind": "declarations", "forwardersExact": True,
         "receiversExact": True, "prepareExact": True,
...
def verify(path, stage, version):
    ...
    if (platform is None or report.get("unityVersion") != version or
            ...
            report.get("profile") != "metadata-forwarding" or
            report.get("platform") != platform):
        raise ValueError("Metadata-forwarding report has the wrong target or stage")
    expected = observations()
    if not same_typed_value(report.get("observations"), expected):
        raise ValueError("Metadata-forwarding behavior differs from the oracle")
    return {"status": "passed", "observations": len(expected), "methods": 5,
            "platform": platform, "profile": "metadata-forwarding",
            "scope": "preparation call order, static field forwarding, cold and repeated calls, mutation, null, signed values and preparation exception"}
