...
Many options, such as `--analysis-level`, `--skip-analysis`, etc, have been removed. Analysis is now off by default and can be enabled with `--output-as dll_il_recovery`.
...
Analysis is now based on an intermediate representation called ISIL (Instruction-Set-Independent Language), which is converted into a Control Flow Graph for more intelligent analysis.
...
Assuming you have a single APK file (not an APKM), and are running at least cpp2il 2021.4.0, you can use the
...
|      --output-as      |        dll_il_recovery        |                                                                          Specify the output format you wish to use.                                                                          |
