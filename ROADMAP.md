...
An exact seven- or eight-instruction x64 proof now accepts a guarded `System.String` field read from an unchanged sealed reference type, directly through a supplied owner or through one unchanged reference field of `this`. It binds the 64-bit load, branch, nonreturning null helper, native unwind, unique managed method, field layout and receiver provenance before allowing managed `ldfld` to retain the null failure. It does not authorize reference-field writes. All four selected methods pass strict player-only recovery, typed IL and both zero-difference declaration gates. Original and recovered Windows editor and Release IL2CPP player runs each pass ten observations, including noninterned reference identity, empty and null string values, and null owners at both levels. Other reference types, deeper access chains and different native shapes remain unverified.
