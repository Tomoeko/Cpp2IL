...
The same synthetic class now adds a signed `int` instance-field write. The exact player exposes a null-throw arm followed by a 32-bit store on the non-null path. A shared field-access proof requires the exclusive throw arm, unchanged managed receiver/field/value binding, and the surviving `stfld` or `ldfld` at emission; the store value is restricted to a bound managed parameter. All three selected methods pass strict player-only recovery, typed IL, and both independent declaration comparisons with zero differences. Original and recovered Windows editor and Release IL2CPP player runs each pass 12 observations, including null receivers and signed boundaries; the rebuilt source contains the field assignment without manual edits. The new claim covers this bounded ordinary store, not other field types, concurrent/volatile semantics, exception messages or stack traces.
