...
A synthetic signed `int` instance-field read now passes strict player-only recovery for both its constructor and read method, typed IL verification, zero-difference declaration comparisons before and after rebuilding, exact Windows editor compilation and a fresh Release IL2CPP player rebuild. Original and recovered editor and player runs each pass six independent observations, including a null receiver and signed boundaries. The bounded rewrite requires an exclusive native null-throw arm, an unchanged managed receiver and field layout, and a field load that survives later analysis so managed `ldfld` retains the null failure. It does not establish field writes, other types, concurrent/volatile semantics, exception messages or stack traces.
...
Support claims must state the exact tested profile and corpus, declaration fidelity, source compilation and native-build results, behavioral coverage, and unresolved counts. Seventy-three methods across fifteen separate fixtures have verified recovered Unity projects and native behavioral receipts: the earlier receipt-driven refresh covered seventy methods in thirteen scopes, the array-read scope adds one, and the field-read scope adds two. There is currently no measured 1:1 accuracy result for a broader corpus or the private target.
