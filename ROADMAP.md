...
A new synthetic array-read control passes the exact Windows editor and original Release IL2CPP player with 28 observations each, including null arrays, negative indices and upper-bound failures. Native inspection shows distinct null and bounds exception exits. Player-only recovery and a rebuilt-source round trip remain unverified for this fixture; its original-player baseline is not counted among the accepted recovered methods.
...
Support claims must state the exact tested profile and corpus, declaration fidelity, source compilation and native-build results, behavioral coverage, and unresolved counts. Seventy methods across thirteen separate fixtures have verified recovered Unity projects and native behavioral receipts; the current committed CLI passes strict recovery and typed IL for all thirteen scopes with unchanged source and method projections. There is currently no measured 1:1 accuracy result for a broader corpus or the private target.
