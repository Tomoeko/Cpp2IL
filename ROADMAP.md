...
The next refresh, after preserving unproved initialization/exception calls and adding bounded floating/byte-field support, still rejects both complete scopes. The private scope contains 1,672 emitted, 4,561 failed, six partial and 1,017 bodyless methods. The independent scope contains 4,415 emitted, 7,655 failed, 175 partial and 479 bodyless methods. All selected and full-input identities remain present. Relative to the preceding refresh, 11/29 methods become emitted while 213/461 formerly emitted methods fail unresolved-local typing. These are analysis dispositions only: neither run produces an accepted source project, and no typed-IL or behavioral success is inferred from the emitted count.
