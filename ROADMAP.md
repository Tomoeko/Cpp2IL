...
Support claims must state the exact tested profile and corpus, declaration fidelity, source compilation and native-build results, behavioral coverage, and unresolved counts. Seventy-eight methods across fifteen separate fixtures have verified recovered Unity projects and native behavioral receipts: the earlier receipt-driven refresh covered seventy methods in thirteen scopes, the array-access scope adds two, and the signed field-access scope adds six. There is currently no measured 1:1 accuracy result for a broader corpus or the private target.
