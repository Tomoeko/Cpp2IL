...
An exact seven- or eight-instruction x64 proof accepts a guarded `System.String` field read from an unchanged reference type, directly through a supplied owner or through one unchanged reference field of `this`. It binds the 64-bit load, branch, nonreturning null helper, native unwind, unique managed method, field layout and receiver provenance before allowing managed `ldfld` to retain the null failure. Derived runtime owners retain the base field identity and offset; reference-field writes, deeper access chains and different native shapes remain unverified. The expanded exact-target fixture includes base and derived owners and retains two derived marker fields. The original Windows editor and Release IL2CPP player each pass sixteen observations, including noninterned reference identity, empty and null strings, null owners and unchanged derived markers.
...
Support claims must state the exact tested profile and corpus, declaration fidelity, source compilation and native-build results, behavioral coverage, and unresolved counts. Ninety-nine methods across seventeen separate fixtures have verified recovered Unity projects and native behavioral receipts: the earlier receipt-driven refresh covered seventy methods in thirteen scopes, the array-access scope adds two, and the signed, Boolean, direct-zero and nested field-access scope adds fourteen, the array-returning call scope adds three, and the expanded string-field scope adds ten. There is currently no measured 1:1 accuracy result for a broader corpus or the private target.
