...
/// <summary>
/// Removes only an exact metadata-literal guard whose later native load becomes the same ldstr.
/// This deliberately excludes general byte tests, type initializers and arbitrary control flow.
/// </summary>
internal static class X86MetadataGuardProof
{
    ...
    public static LiteralGuard? Find(MethodAnalysisContext context, IReadOnlyList<Instruction> body)
    {
        ...
        addresses.Remove(0);
        return Find(body, addresses, address => context.AppContext.LibCpp2IlContext.GetLiteralByAddress(address) != null);
    }
        ...
        Func<ulong, bool> isLiteralSlot)
    {
        for (var index = 0; index + 5 < body.Count; index++)
        {
            ...
            if (compare.Mnemonic != Mnemonic.Cmp || !IsRipMemory(compare, 0, 1) ||
            ...
            if (branch.Mnemonic != Mnemonic.Jne || branch.Op0Kind != OpKind.NearBranch64 ||
                branch.NearBranchTarget != materialize.IP ||
                argument.Mnemonic != Mnemonic.Lea || argument.Op0Register != Register.RCX || !IsRipMemory(argument, 1) ||
                call.Mnemonic != Mnemonic.Call || call.Op0Kind != OpKind.NearBranch64 || !helperAddresses.Contains(call.NearBranchTarget) ||
                ...
                materialize.Mnemonic != Mnemonic.Mov || materialize.Op0Register != Register.RAX || !IsRipMemory(materialize, 1, 8) ||
